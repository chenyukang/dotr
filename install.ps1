param(
    [string]$Version = $env:DOTR_VERSION,
    [string]$InstallDir = $env:DOTR_INSTALL_DIR
)
...
function Save-Download {
    ...
    Write-Host "dotr install: downloading $Label"
...
$TempDir = Join-Path ([System.IO.Path]::GetTempPath()) "dotr-install-$([System.Guid]::NewGuid())"
...
try {
    ...
    if (-not $PathContainsInstallDir) {
        ...
        if (-not $UserPathContainsInstallDir) {
            ...
            Write-Host "dotr install: added $ResolvedInstallDir to your user PATH"
            Write-Host "Open a new terminal before running dotr."
        }
    }

    Write-Host "dotr install: installed $Tag to $Destination"
    Write-Host "Run: dotr --help"
} finally {
