...
repo="chenyukang/dotr"
bin_name="dotr"
version="${DOTR_VERSION:-${1:-latest}}"
install_dir="${DOTR_INSTALL_DIR:-${BIN_DIR:-$HOME/.local/bin}}"

usage() {
  cat <<'EOF'
Install dotr from GitHub release binaries.
...
Usage:
  ...
  curl -fsSL https://raw.githubusercontent.com/chenyukang/dotr/main/install.sh | DOTR_INSTALL_DIR=/usr/local/bin sh

Environment:
  DOTR_VERSION      Release tag to install. Defaults to latest.
  DOTR_INSTALL_DIR  Install directory. Defaults to ~/.local/bin.
...
need_cmd() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "dotr install: required command not found: $1" >&2
    exit 1
  fi
}
...
fetch_to_file() {
  ...
  if command -v curl >/dev/null 2>&1; then
    ...
    echo "dotr install: curl or wget is required" >&2
    exit 1
  fi
}
...
detect_target() {
  ...
  case "$kernel" in
    ...
    Linux)
      ...
      ;;
    *)
      echo "dotr install: unsupported OS: $kernel" >&2
      exit 1
      ;;
  esac
  ...
  case "$machine" in
    ...
    arm64|aarch64)
      ...
      ;;
    *)
      echo "dotr install: unsupported architecture: $machine" >&2
      exit 1
      ;;
  esac
...
sha256_file() {
  ...
  if command -v sha256sum >/dev/null 2>&1; then
    ...
    echo "dotr install: sha256sum or shasum is required to verify the download" >&2
    exit 1
  fi
}
...
tmp_dir="$(mktemp -d 2>/dev/null || mktemp -d -t dotr-install)"
...
if [ "$version" = "latest" ]; then
  ...
  if [ -z "$tag" ]; then
    echo "dotr install: failed to resolve latest release tag" >&2
    exit 1
  fi
else
  case "$version" in
...
echo "dotr install: downloading $archive"
if ! download_to_file "$base_url/$archive" "$archive_path"; then
  echo "dotr install: release asset not found: $base_url/$archive" >&2
...
if ! fetch_to_file "$base_url/$archive.sha256" "$checksum_path"; then
  echo "dotr install: checksum asset not found: $base_url/$archive.sha256" >&2
  exit 1
fi
...
if [ "$expected" != "$actual" ]; then
  echo "dotr install: checksum mismatch for $archive" >&2
...
if [ ! -x "$source_bin" ]; then
  echo "dotr install: archive did not contain executable $bin_name" >&2
  exit 1
fi
...
case ":$PATH:" in
  *":$install_dir:"*) ;;
  *)
    echo "dotr install: installed to $install_dir, which is not in PATH" >&2
    echo "Add this to your shell profile: export PATH=\"$install_dir:\$PATH\"" >&2
    ;;
esac

echo "dotr install: installed $tag to $install_dir/$bin_name"
echo "Run: dotr --help"
